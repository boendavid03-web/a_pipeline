...
DURATION_SEC="${DR_SPAAM_TRACKING_DURATION_SEC:-75}"
DOMAIN_ID="${DR_SPAAM_TRACKING_DOMAIN_ID:-80}"
RUN_ID="$(date +%Y%m%d_%H%M%S)"
RUN_DIR="${DR_SPAAM_TRACKING_RUN_DIR:-$PROJECT_ROOT/runs/dr_spaam_isaac_tracking_smoke/$RUN_ID}"
...
for required in \
    ...
    "$CHECKPOINT"; do
    if [[ ! -e "$required" ]]; then
        echo "ERROR: required tracking smoke input is missing: $required" >&2
        exit 1
    fi
done
if ! awk -v value="$DURATION_SEC" 'BEGIN { exit !(value ~ /^[0-9]+([.][0-9]+)?$/ && value >= 10) }'; then
    echo "ERROR: DR_SPAAM_TRACKING_DURATION_SEC must be at least 10 seconds." >&2
    exit 2
fi
if [[ ! "$DOMAIN_ID" =~ ^[0-9]+$ ]] || (( DOMAIN_ID > 232 )); then
    echo "ERROR: DR_SPAAM_TRACKING_DOMAIN_ID must be an integer from 0 through 232." >&2
    exit 2
fi
...
echo "DR_SPAAM_TRACKING_RUN_DIR=$RUN_DIR"
echo "ROS_DOMAIN_ID=$ROS_DOMAIN_ID"
...
if [[ -f "$RUN_DIR/evaluation/summary.json" ]]; then
    echo "DR_SPAAM_TRACKING_SUMMARY=$RUN_DIR/evaluation/summary.json"
else
    echo "ERROR: tracking evaluator summary was not produced." >&2
    exit 8
fi
exit "$isaac_status"
