...
PYTHON_SCRIPT="$SCRIPT_DIR/show_ira_people_6_0.py"
...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/ira_people_6_0_$(date +%Y%m%d_%H%M%S).log"
...
if rg -i 'segmentation fault|fatal signal|libomni\.anim\.skelJoint|crash detected' "$LOG_FILE" >/dev/null 2>&1; then
    echo "WARNING: a possible native crash signature was found in $LOG_FILE" >&2
    exit 2
fi
