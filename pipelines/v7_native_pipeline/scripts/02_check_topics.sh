...
# 可能使用的关键环境变量：BASH_SOURCE, IGN_PARTITION, MISSING, PASS, ROS_DOMAIN_ID, RUN_MANIFEST, RUN_ROOT, SCRIPT_DIR
# 数据说明：以上内容从代码正文中的参数、topic、消息导入、文件扩展名和环境变量提取；实际字段、shape、单位和发布方向仍以本脚本正文为准。
# 【后续管理信息】
# 类型：Shell 流程脚本
# 推荐运行方式：bash /home/user/navigation_project/a_pipeline/pipelines/v7_native_pipeline/scripts/02_check_topics.sh
...
# 【绝对依赖关系】
# 文件绝对路径：/home/user/navigation_project/a_pipeline/pipelines/v7_native_pipeline/scripts/02_check_topics.sh
...
LOG="${RUN_ROOT}/logs/02_check_topics_$(timestamp).log"

{
  ...
  for topic in /scan_merged /scan_01 /scan_02 /odom /tf /tf_static /clock /cmd_vel /map; do
  ...
  echo
  echo "===== topic info ====="
  for topic in /scan_merged /scan_01 /scan_02 /tf_static /odom /cmd_vel; do
    echo "--- ${topic} ---"
    ros2 topic info "${topic}" || true
  done
  echo
  echo "===== /scan_merged hz, 8 seconds ====="
  timeout 8 ros2 topic hz /scan_merged || true
} 2>&1 | tee "${LOG}"
