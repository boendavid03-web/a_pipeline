...
# 可能使用的关键环境变量：ANGLE_MAX, ANGLE_MIN, FRAME_BASE, FRAME_MAP, FRAME_ODOM, FRAME_SCAN_01, FRAME_SCAN_02, FRAME_SCAN_MERGED, LABEL_NAMES, LABEL_PNG, MAP_DIR, MAP_PGM, MAP_YAML, NAVIGATION_PROJECT_ROOT, NUM_FRAMES, ORIGIN_X, ORIGIN_Y, OUTPUT_DIR, PROJECT_ROOT, RANGE_MAX
# 数据说明：以上内容从代码正文中的参数、topic、消息导入、文件扩展名和环境变量提取；实际字段、shape、单位和发布方向仍以本脚本正文为准。
# 【后续管理信息】
# 类型：Python 工具/训练/转换脚本
# 推荐运行方式：python3 /home/user/navigation_project/a_pipeline/workspaces/ros2_ws/tools/generate_sim_raw_v7_semantic2d_seed.py
...
# 被依赖的具体作用：/home/user/navigation_project/a_pipeline/scripts/validation/ros2_workspace_tools/generate_sim_raw_v7_semantic2d_seed.py（引用其脚本路径或名称，形成流程关联）
# 说明依据：上述关系根据本文件中的 source、ros2 launch、ros2 run、import、subprocess 和脚本路径调用整理；参数/话题级关系仍以代码正文为准。
# 【绝对依赖关系】
# 文件绝对路径：/home/user/navigation_project/a_pipeline/workspaces/ros2_ws/tools/generate_sim_raw_v7_semantic2d_seed.py
# 直接依赖（脚本层面）：无项目内脚本引用（外部库/ROS 2 命令另见代码正文）。
# 被依赖/被引用（脚本层面）：/home/user/navigation_project/a_pipeline/scripts/validation/ros2_workspace_tools/generate_sim_raw_v7_semantic2d_seed.py
...
PROJECT_ROOT = Path(
    os.environ.get("NAVIGATION_PROJECT_ROOT", Path(__file__).resolve().parents[3])
).resolve()
WORKSPACE = PROJECT_ROOT / "workspaces" / "ros2_ws"
MAP_DIR = WORKSPACE / "maps" / "semantic_labeling_v6"
