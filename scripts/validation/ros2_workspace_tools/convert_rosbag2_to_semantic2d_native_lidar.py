#!/usr/bin/env python3
# 【具体数据接口】
# 代码中检测到的命令行参数：--bag, --base-frame, --cmd-vel-stamped-topic, --cmd-vel-topic, --dev-ratio, --map-frame, --map-yaml, --odom-topic, --output-root, --overwrite, --pose-source, --rewrite-dataset-index, --scan-topic, --semantic-label, --session-name, --split-seed, --strict-tf, --subgoal-lookahead, --test-ratio, --train-ratio, --write-projection-debug
...
# 可能使用的关键环境变量：CMD_VELOCITY_DIM, DEFAULT_LABEL_NAMES, IGNORE_LABEL, POSE_SOURCES, WARNING
# 数据说明：以上内容从代码正文中的参数、topic、消息导入、文件扩展名和环境变量提取；实际字段、shape、单位和发布方向仍以本脚本正文为准。
# 【后续管理信息】
# 类型：Python 工具/训练/转换脚本
# 推荐运行方式：python3 /home/user/navigation_project/a_pipeline/scripts/validation/ros2_workspace_tools/convert_rosbag2_to_semantic2d_native_lidar.py
...
def semantic_for_scan(ranges, angles, valid_mask, pose, label_img, resolution, origin_x, origin_y):
    ...
    labels[project_indices[in_map]] = label_img[rows[in_map], cols[in_map]].astype(np.int64)
    return labels
    ...
    for sample_index in sample_indices:
        ...
        if len(project_indices) > 0:
            ...
            for idx, col, row in zip(project_indices[in_map], cols[in_map], rows[in_map]):
                color = (255, 0, 0) if int(semantic[idx]) == IGNORE_LABEL else (0, 255, 255)
                for dx in (-1, 0, 1):
                    for dy in (-1, 0, 1):
                        xx = int(col) + dx
                        yy = int(row) + dy
...
def main():
    ...
    for stamp_ns, scan_msg in scans:
        ...
        semantic = semantic_for_scan(
            scan, angles, valid_mask, pose, label_img, resolution, origin_x, origin_y
        )
